...
void road(int N, int S, int E, int W){
	...
	while(S > 0){
		std::jthread pusht1(out, 0, S, 0, 0);
		S--;
	}
	while(N > 0){
		std::jthread pusht(out, N, 0, 0, 0);
		N--;
	}
	while(W > 0){
		std::jthread pusht2(out, 0, 0, W, 0);
		W--;
	}
	while(E > 0){
		std::jthread pusht3(out, 0, 0, 0, E);
		E--;
	}
...
int main(){
	std::deque<int> t = {1, 1, 1, 1};
	std::deque<int> ss = {1, 2, 1, 2};
	int o = 1;
	while(!t.empty()){
		if(t[0] == t[1]){
			o++;
